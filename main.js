document.addEventListener("DOMContentLoaded", function(event) {
	...
	function playNote(note) {
	    ...
	    gainNodeAI.gain.setTargetAtTime(0.8, note.startTime+offset, 0.01)
	    oscAI.frequency.setTargetAtTime(midiToFreq(note.pitch), note.startTime+offset, 0.001)
	    gainNodeAI.gain.setTargetAtTime(0, note.endTime+offset-0.05, 0.01)
	...
	playAI.addEventListener('click', function() {
	    ...
	    oscAI = audioCtxAI.createOscillator();
	    gainNodeAI = audioCtxAI.createGain();
	    oscAI.connect(gainNodeAI).connect(audioCtxAI.destination);

	    var modulatorFreq = audioCtxAI.createOscillator();
	    modulatorFreq.frequency.value = 1;

	    const modulated = audioCtxAI.createGain();
	    const depth = audioCtxAI.createGain();
	    depth.gain.setValueAtTime(0.01, audioCtx.currentTime);
	    modulated.gain.setValueAtTime(0.01, audioCtx.currentTime);
	    depth.gain.setTargetAtTime(0.5, audioCtx.currentTime, 0.1);
	    modulated.gain.setTargetAtTime(0.9 - depth.gain.value, audioCtx.currentTime, 0.1);
	    ...
	    modulatorFreq.connect(depth).connect(modulated.gain);
	    oscAI.connect(modulated)
	    modulated.connect(gainNodeAI).connect(audioCtxAI.destination);
	    
	    oscAI.start();
	    modulatorFreq.start();
	    gainNodeAI.gain.value = 0;
